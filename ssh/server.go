...
// InitializeSSHServerConfig initializes the SSH server configuration.
// Loads or generates the host key, sets up password authentication, and returns the config.
func InitializeSSHServerConfig() (*ssh.ServerConfig, error) {
	keyPath := "host_key"
	...
	if err != nil {
		...
		privateKey, err := GenerateRSAPrivateKey(4096)
		if err != nil {
			return nil, fmt.Errorf("Failed to generate private key: %v", err)
		}
		privateBytes = EncodeRSAPrivateKeyToPEM(privateKey)
...
// HandleIncomingSSHConnection handles an incoming SSH connection.
// Accepts the connection, processes channels, and ensures proper cleanup.
func HandleIncomingSSHConnection(conn net.Conn, config *ssh.ServerConfig) {
	...
	HandlePortForwardChannels(chans)
